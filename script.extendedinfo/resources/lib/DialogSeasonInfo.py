...
def get_season_window(window_type):
	...
	class DialogSeasonInfo(DialogBaseInfo, window_type):
		...
		def browse_season(self):
			url = 'plugin://plugin.video.chappaai/tv/tvdb/%s/%s/' % (self.info['tvdb_id'], self.info['season'])
			self.close()
		...
		def play_season(self):
			url = 'plugin://plugin.video.chappaai/tv/play/%s/%s/1' % (self.info['tvdb_id'], self.info['season'])
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.action('contextmenu', 10)
		def play_season_choose_player(self):
			url = 'plugin://plugin.video.chappaai/tv/play_choose_player/%s/%s/1/False' % (self.info['tvdb_id'], self.info['season'])
			xbmc.executebuiltin('RunPlugin(%s)' % url)
