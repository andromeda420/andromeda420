	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def play_movie(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.chappaai/movies/play/tmdb/%s' % self.info.get('id', '')
		...
		def play_movie_choose_player(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.chappaai/movies/play_choose_player/tmdb/%s/False' % self.info.get('id', '')
				xbmc.executebuiltin('RunPlugin(%s)' % url)
		...
		def add_movie_to_library(self):
			...
			if xbmcgui.Dialog().yesno('Wraith', 'Add [B]%s[/B] to library?' % self.info['title']):
		...
		def remove_movie_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.chappaai)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.chappaai/setup/total)')
			if xbmcgui.Dialog().yesno('Wraith', 'Remove [B]%s[/B] from library?' % self.info['title']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.CHAPPAAI_MOVIE_FOLDER, self.info['imdb_id']))):
					Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %d}' % int(self.info['dbid']))
					shutil.rmtree(xbmc.translatePath('%s%s/' % (Utils.CHAPPAAI_MOVIE_FOLDER, self.info['imdb_id'])))
					Utils.after_add(type='movie')
