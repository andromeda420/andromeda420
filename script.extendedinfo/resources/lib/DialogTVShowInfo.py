...
def get_tvshow_window(window_type):
	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def browse_tvshow(self):
			url = 'plugin://plugin.video.chappaai/tv/tvdb/%s/' % self.info['tvdb_id']
			self.close()
		...
		def show_manage_dialog(self):
			manage_list = []
			manage_list.append(["Wraith's settings", 'Addon.OpenSettings("script.extendedinfo")'])
			manage_list.append(["Chappaai's settings", 'Addon.OpenSettings("plugin.video.chappaai")'])
		...
		def play_tvshow(self):
			url = 'plugin://plugin.video.chappaai/tv/play/%s/1/1' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.action('contextmenu', 9)
		def play_tvshow_choose_player(self):
			url = 'plugin://plugin.video.chappaai/tv/play_choose_player/%s/1/1/False' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.click(20)
		def add_tvshow_to_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.chappaai)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.chappaai/setup/total)')
			if xbmcgui.Dialog().yesno('Wraith', 'Add [B]%s[/B] to library?' % self.info['TVShowTitle']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.chappaai/tv/add_to_library/%s)' % self.info['tvdb_id'])
		...
		def remove_tvshow_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.chappaai)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.chappaai/setup/total)')
			if xbmcgui.Dialog().yesno('Wraith', 'Remove [B]%s[/B] from library?' % self.info['TVShowTitle']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.CHAPPAAI_TV_FOLDER, self.info['tvdb_id']))):
					Utils.get_kodi_json(method='VideoLibrary.RemoveTVShow', params='{"tvshowid": %s}' % int(self.info['dbid']))
					shutil.rmtree(xbmc.translatePath('%s%s/' % (Utils.CHAPPAAI_TV_FOLDER, self.info['tvdb_id'])))
					Utils.after_add(type='tv')
