...
from resources.lib.TraktManager import *
...
def get_tmdb_window(window_type):
	...
	class DialogVideoList(DialogBaseList, window_type):
		...
		def context_menu(self):
			...
			else: imdb_id = TheMovieDB.get_imdb_id_from_movie_id(item_id)
			...
			if selection == 1:
				if self.listitem.getProperty('TVShowTitle'):
					TVLibrary = xbmcaddon.Addon('plugin.video.chappaai').getSetting('tv_library_folder')
					if self.listitem.getProperty('dbid'):
						...
						if xbmcgui.Dialog().yesno('Wraith', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('TVShowTitle')):
					...
					if self.listitem.getProperty('dbid'):
						if xbmcgui.Dialog().yesno('Wraith', 'Remove [B]%s[/B] from library?' % self.listitem.getProperty('title')):
							Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %s}' % dbid)
							MovieLibrary = xbmcaddon.Addon('plugin.video.chappaai').getSetting('movies_library_folder')
						...
						if xbmcgui.Dialog().yesno('Wraith', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('title')):
