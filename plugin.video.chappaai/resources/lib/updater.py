...
def update_players_local(zip_path):
	extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.chappaai/Players')  
...
def flat_extract(z, extract_to, members=None):
	...
	if not os.path.exists(extract_to):
		...
		if xbmc.getInfoLabel('Window(home).Property(running)') == 'totalchappaai':
			...
			if plugin.yesno('Chappaai: Update players', 'Do you want to remove existing players first?'):
				empty_folder(extract_to)
